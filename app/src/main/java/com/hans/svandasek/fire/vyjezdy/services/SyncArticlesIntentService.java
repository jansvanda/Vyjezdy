...
import com.hans.svandasek.fire.vyjezdy.receivers.SyncArticlesReceiver;
    ...
    private void parseWebArticle(String[] links, int position) {
        if (position != links.length - 1 && STATUS) {
            try {
                ...
                Elements paragraphs = htmlDocument.select("p:contains(Popis), p:contains(Jednotky), p:contains(Obec), p:contains(Část Obce), p:contains(Ukončená), p:contains(SaP)");
                String body = "";
